...
Опциональные атрибуты:
* `mount` — описывает точку монтирования во вложенном атрибуте `point` (точка должна быть пропиана в fstab) и опционально команду для монтирования в `cmd`.
...
Описывает способ выгрузки архивов на FTP-сервер, используя команды и пакета ncftp.
